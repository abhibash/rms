...
function selectRestaurant() {
  ...
  if (!isEmpty(order)) {
    result = confirm(
      "Are you sure you want to clear your order and switch menus?"
    );
  }
  ...
  if (result) {
    ...
    document.getElementById("left").innerHTML =
      getCategoryHTML(currentRestaurant);
    document.getElementById("middle").innerHTML =
      getMenuHTML(currentRestaurant);
    ...
    info.innerHTML =
      ...
      "<br>Minimum Order: $" +
      currentRestaurant.min_order +
      "<br>Delivery Fee: $" +
      currentRestaurant.delivery_fee +
      "<br><br>";
  } else {
...
function updateOrder() {
  ...
  Object.keys(order).forEach((id) => {
    ...
    result += `${item.name} x ${order[id].quantity} (${(
      item.price * order[id].quantity
    ).toFixed(
      2
  ...
  currentTotal =
    currentSubtotal + currentSubtotal * 0.1 + currentRestaurant.delivery_fee;
  ...
  if (currentSubtotal >= currentRestaurant.min_order) {
    ...
    result += `Add \$${(currentRestaurant.min_order - currentSubtotal).toFixed(
      2
    )} more to your order.`;
  }
